...
class Dataset_Human(Dataset):
    ...
    def __read_data__(self):
        ...
        # df_raw = np.load('/home/guest/zkt/IDOL/human/Discussion_all.npy')
        '''
        df_raw.columns: ['date', ...(other features), target feature]
        '''
        num_train = int(len(df_raw) * 0.7)
...
class Dataset_Humaneva(Dataset):
    ...
    def __read_data__(self):
        ...
        df_raw = np.load(os.path.join(self.root_path, self.data_path))
        # print(df_raw.shape)
        # rows_with_nan = np.isnan(df_raw).any(axis=1)
        # # 删除包含空值的行
        # df_cleaned = df_raw[~rows_with_nan]
